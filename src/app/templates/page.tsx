...
const TemplatesPage = () => {
  ...
  const handleBuyNowClick = async (template: Template) => {
    setSelectedTemplate(template);
    await fetchPackagesForTemplate(template.id);
    setIsModalOpen(true);
  };

  const handlePackageSelect = (packageData: Package, template: Template) => {
                  ...
                  <div className={`p-6 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                    ...
                    <div className="flex gap-3">
                      <button 
                        ...
                        className="flex-1 bg-gradient-to-r from-[#52303f] to-[#7c5367] text-white px-4 py-3 rounded-lg font-semibold hover:from-[#7c5367] hover:to-[#52303f] transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transform hover:-translate-y-1"
                      >
